...
IH_ENTRY:			// PC, ST, PSR are saved in iRTN, iST, iPSR
	...
	SWAP	A, SP	// A를 SP에 저장
			// A는인터럽트 걸린 프로그램에서 사용 중일 수 있으므로 보존해야 함
			// SP는 iSP에 저장되어 있고으므로 손상되어도 됨
	LD	A, S_STACK_BTM	// 시스템스택포인터를 A에 읽어옴
	SWAP	A, SP	// A를 복원하면서 동시에 SP를 시스템스택으로 세팅
...
SWITCH_TASK:			
	...
	MOV	A, #9	// will move 9 visible registers
	MOV	C, A	
...
SAVE_CURR_TASK:			
	...
	LD	A, TCB_LENGTH	
	IMUL	A, D3	// D3: index for the current task
	ADD	D1, A	// D1: Block for Visible Reg of Current Task
	MOV	B, D1	
			
	CLR	X	
	STCTX	@%(-_CTX_SIZE)	// Save iRTN, iPSR, iSP to TCB
	BLKMOV	@*D1 @*D0	// Block copy from D0 to D1 (C: counter)
RESTORE_NEW_TASK:			
	LD	A, P_TCB_VISIBLE	
	MOV	D1, A	
	LD	A, TCB_LENGTH	
	IMUL	A, D2	// D2: index for a new task
	ADD	D1, A	// D1: Block for Visible Reg of a new Task
	MOV	B, D1	
